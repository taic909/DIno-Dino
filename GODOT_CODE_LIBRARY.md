...
Terrain impacts now end the active Glide instead of letting stored Glide momentum restore full speed on the next frame. A wall hit close to parallel (at most 0.3 of travel directed into the wall by default) gives a reduced bounce; a more direct hit keeps only 20% of the sliding motion plus a small push away from the wall. Floor and ceiling hits use the hard-impact rule without a wall bounce. In the Player Inspector, `glide_wall_glance_max_incidence` controls which angles count as a graze, `glide_wall_glance_speed_retention` sets its surviving speed, and `glide_hard_impact_speed_retention` sets the heavier penalty. These values are based on the collision normal reported by `move_and_slide()`, not the sprite's rotation.
